...
### [Auric 睿云桥（88token）](https://88token.net/)
Auric 睿云桥主要面向开发者提供 AI API 接入。站内支持 API 密钥创建与管理，可以查看每次调用记录和消费明细；除了接口调用，也提供网页端在线创作工具，包含聊天、图片和视频任务。另外还有邀请返利和 Token 用量榜，遇到问题可以通过站内留下的联系方式咨询。

模型线路和高峰期表现可能会随实际渠道变化，不建议一开始就大额充值。可以先小额测试自己的真实场景，重点看看常用模型的响应速度、成功率、输出效果和扣费记录，确认适合后再逐步增加用量。
